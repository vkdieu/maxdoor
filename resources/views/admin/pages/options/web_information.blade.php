...
@section('content')
  ...
  <section class="content">
    ...
    <div class="box box-primary">
      ...
      <form role="form" action="{{ route('web.information.update', $detail->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="box-body">
          @foreach (json_decode($detail->option_value) as $key => $value)
            <div class="col-md-12">
              <div class="form-group">
                <label for="option_value_{{ $key }}">{{ __($key) }}</label>
                <input type="text" class="form-control" name="option_value[{{ $key }}]"
                  id="option_value_{{ $key }}" value="{{ $value }}" placeholder="{{ __($key) }}">
              </div>
            </div>
          @endforeach
        </div>
